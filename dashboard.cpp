...
QList<QList<QString>> Dashboard::getInventory() {
    ...
    for(int i=0; i < inventory.count(); i++) {
        ...
        if(index == -1 && inventory.at(i).at(0) != NULL) {
            ...
            temp.push_back(QString::number((inventory.at(i).at(3).toDouble() * inventory.at(i).at(4).toInt()), 'f', 2));
        ...
        // update current item if not a NULL item
            ...
            combined[index][2] = QString::number(combined.at(index).at(2).toInt() + inventory.at(i).at(4).toInt());
            combined[index][3] = QString::number((combined.at(index).at(2).toInt() * inventory.at(i).at(3).toDouble()), 'f', 2);
        }
    }
